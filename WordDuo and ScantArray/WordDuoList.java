...
public class WordDuoList {
    ...
    public WordDuoList(String[] words){
        ...
        for(int i = 0; i < words.length - 1; i++){
            for(int j = i + 1; j < words.length; j++){
                WordDuo duo = new WordDuo(words[i], words[j]);
                allDuos.add(duo);
            }
        }
    }
    ...
    public void moveMatchesToTop(){
        ...
        for (int i = 0; i < allDuos.size(); i++) {
            WordDuo currentDuo = allDuos.get(i); 
            if (currentDuo.getFirst().equals(currentDuo.getSecond())) {
                allDuos.add(index, allDuos.remove(i));
                index++;
            }
        }
    }
